<template>
  ...
  >
    ...
    >
      ...
      <el-form-item
        class="secrecyAgreement"
        prop="secrecyAgreement"
        label="在原单位是否签订过与我司所任此职位相关的保密协议"
      >
...
export default {
    ...
    dataRule() {
      ...
      var validateEmail = (rule, value, callback) => {
        if (value && !isEmail(value)) {
          return callback(
            new Error(this.$t("validate.format", { attr: this.$t("user.email") }))
          );
        }
        callback();
      };
      var validateMobile = (rule, value, callback) => {
        if (value && !isMobile(value)) {
          return callback(
            new Error(this.$t("validate.format", { attr: this.$t("user.mobile") }))
          );
        }
        callback();
      };
  ...
  methods: {
    ...
    getInfo() {
      this.$http
        .get(`/workerInfo/detail/${this.dataForm.id}`)
        .then(({ data: res }) => {
          if (res.code !== 0) {
