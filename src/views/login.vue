<template>
  <div class="login-wrap">
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="60%">
      <el-table size="small"  highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
        <el-table-column type="index" label="序号" width="50"></el-table-column>
        <el-table-column sortable prop="IP" label="IP" width="180">
        </el-table-column>
        <el-table-column sortable prop="loginTime" label="登录时间" width="180">
        </el-table-column>
        <el-table-column sortable prop="remark" label="备注" width="220">
        </el-table-column>
        <el-table-column align="center" label="操作" min-width="100">
          <template slot-scope="scope">
            <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">中断</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div style="display:flex;justify-content:center;margin-top:30px;"><el-button type="primary" style="width:15%;" @click="pleaseLogin">请登录</el-button></div>
    </el-dialog>
  </div>
</template>
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          //   // this.getdata();
          //   // this.$router.push({ path: '/interface/Interface' })
          // }, 1000)
          // 注释
          login(this.ruleForm).then(res => {
            if (res.code == 200) {
              ...
              //   this.loading = false
                ...
                // this.$router.push({ path: '/interfaceList/InterfaceList' })
              // }, 1000)
            } else {
    ...
    getdata(parameter) {
      ...
      let res = {
        ...
        data: [
          {
            IP:'10.44.8.250',
            loginTime:'2021-07-06 14:14:43',
            remark:''
          },
          {
            IP:'10.43.89.110',
            loginTime:'2021-07-06 15:17:55',
            remark:''
          },
        ]
      }
